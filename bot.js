(() => {
  ...
  const LANGS = {
    vi: {
      ...
      preview:'Xem trước (lớp phủ)',
      start:'Bắt đầu',
      pause:'Tạm dừng',
      resume:'Tiếp tục',
      stop:'Dừng',
      builtQueue:'Hàng đợi tạo: {n} px',
      needImgPos:'Tải ảnh lên và chọn vị trí trên canvas.',
      waitingClick:'Nhấp vào GÓC TRÁI TRÊN của tác phẩm trong canvas.',
      posOK:'Căn chỉnh tại X:{x} Y:{y}.',
      loadOK:'Ảnh: {w}×{h} • {n} px',
      overlayOn:'Lớp phủ BẬT.',
      overlayOff:'Lớp phủ TẮT.',
      done:'✅ Hoàn thành! Pixels: {n}',
      paused:'⏸️ Đã tạm dừng.',
      resumed:'▶️ Đang tiếp tục.',
      stopped:'⏹️ Đã dừng.',
      committing:'⏳ Đang áp dụng…',
      committed:'✅ Đã áp dụng.',
      sessionSaved:'💾 Phiên làm việc đã lưu.',
      sessionLoaded:'📦 Phiên làm việc đã khôi phục.',
      toastHit:'⚠️ Hết mực — đang hợp nhất…',
      coolingDown:'🧊 Đang làm mát {min}phút… còn lại {mmss}',
      noCanvas:'Không tìm thấy canvas. Mở trang bản đồ.',
      openPalette:'Mở bảng màu của trang web.',
      nothingToPaint:'Không có gì để vẽ (bộ lọc hiện tại).',
      started:'🚀 Đang vẽ…',
      mustPickPos:'Chọn vị trí trước khi bắt đầu.',
      mustUpload:'Tải ảnh lên trước khi bắt đầu.',
      cooldownLabel:'Thời gian nghỉ sau khi cạn (phút)',
      reopenNormal:'Mở lại bảng màu sau khi commit (ms)',
      reopenDepl:'Mở lại sau khi cạn (ms)',
      speed:'Tốc độ & Độ chính xác',
      pixelSize:'Kích thước pixel',
      alpha:'Độ trong suốt <',
      white:'Trắng ≥',
      skipWhite:'Bỏ qua trắng',
      skipAlpha:'Bỏ qua trong suốt',
      order:'Thứ tự',
      scan:'Quét dòng',
      serp:'Rắn',
      center:'Tâm→viền',
      ...
      processed:'Đã xử lý',
      of:'/',
      apiFreeHint:'Không API: vẽ đến khi hiện toast "Acabou a tinta/Out of paint". Khi xảy ra, commit, chờ +1 (~35s) và làm mát.',
      ...
      helpText:'Mẹo: chọn vị trí, bật lớp phủ để kiểm tra và nhấp Bắt đầu. Lỗi hiện dưới dạng thông báo.',
      topToastDemo:'Sẵn sàng. Thông báo quan trọng hiện ở đây.',
      resizeHint:'Nên đổi kích thước để vừa với bản đồ, nhưng không bắt buộc — thay đổi là trực tiếp.',
      // manual start
      manualStartLabel:'Sử dụng bắt đầu thủ công',
      manualStartIndex:'Pixel bắt đầu (#)',
      manualStartHelp:'Nếu bật, bắt đầu từ chỉ số pixel này.',
      jumpTo:'Bắt đầu thủ công: nhảy đến #{n}',
      posSet:'Đã đặt vị trí tại X:{x} Y:{y}.',
      posSelectCanceled:'Đã hủy chọn vị trí.',
      clickToSetPos:'Nhấp vào canvas để đặt vị trí.',
      canvasNotFound:'Không tìm thấy canvas.'
    },
    pt: {
      title: `FXBot - Pixels ${VERSION}`,
      menu_status: 'STATUS',
      ...
      posSet:'Alinhado em X:{x} Y:{y}.',
      posSelectCanceled:'Seleção de posição cancelada.',
      clickToSetPos:'Clique no canvas para definir posição.',
      canvasNotFound:'Canvas não encontrado.'
    },
    en: {
      title: `FXBot - Pixels ${VERSION}`,
      menu_status: 'STATUS',
      ...
      posSet:'Aligned at X:{x} Y:{y}.',
      posSelectCanceled:'Position selection canceled.',
      clickToSetPos:'Click on canvas to set position.',
  ...
  const detectBrowserLang = () => {
    try{
      ...
      if(langs.some(l => l.startsWith('vi'))) return 'vi';
      if(langs.some(l => l.startsWith('pt'))) return 'pt';
      return 'en';
    }catch{ return 'vi'; } // Default to Vietnamese
  };
  ...
  state._resolvedLang = state.lang === 'auto' ? detectBrowserLang() : (state.lang||'vi');
  if(!(state._resolvedLang in LANGS)) state._resolvedLang = 'vi';
    ...
    const langSel = `
      <label style="display:flex;align-items:center;gap:8px;font-size:12px;color:${THEME.subtle}">
        ...
        <select id="fx-lang" style="background:${THEME.bg};border:1px solid ${THEME.border};color:${THEME.text};border-radius:8px;padding:6px;outline:none">
          ...
          <option value="vi"${state.lang==='vi'?' selected':''}>${t('langVI')}</option>
    ...
    inner.innerHTML = `
      <div id="fx-drag-handle" style="display:flex;align-items:center;gap:10px;justify-content:space-between;padding:10px 14px;border-bottom:1px solid ${THEME.border}; background:rgba(10,10,14,.6); border-top-left-radius:16px;border-top-right-radius:16px;cursor:move">
        ...
        <div style="display:flex;gap:8px;align-items:center">
          ...
          ${langSel}
          <button id="fx-save" class="fx-btn ghost">Lưu</button>
          <button id="fx-restore" class="fx-btn ghost" ${hasSession()?'':'disabled'}>Khôi phục</button>
  ...
  async function onFile(e){
    ...
    fr.onload=()=>{
      ...
      img.onerror=()=>{ setStatus('Lỗi'); showToast('Lỗi khi tải ảnh', 'error'); };
      img.src=fr.result;
    };
  ...
  function resizeImage(){
    ...
    const w=parseInt(prompt('Chiều rộng (px):',state.imgWidth),10);
    const h=parseInt(prompt('Chiều cao (px):',state.imgHeight),10);
  ...
  function startToastObserver(){
    try{
      ...
      const re = /Acabou a tinta|Out of paint|Hết mực/i;
  ...
  async function startPainting(){
    if(state.loopActive){ showToast('Đã đang chạy', 'warn'); return; }
  ...
  async function mainLoopClassic(){
    ...
    while(state.running && !state.stopFlag){
      ...
      setAction(`Màu ${it.colorId} | ${state.queuePtr+1}/${state.queue.length}`);
  ...
  function stopPainting(){
    ...
    // overlay: xóa LUÔN và bỏ listeners
